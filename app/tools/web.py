...
class WebSearchTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        max_results = int(kwargs.get("max_results", self.settings.ddg_max_results))
        results = []
        with DDGS() as ddgs:
        ...
        lines = []
        for item in results:
            lines.append(f"{item.get('title', 'Untitled')} | {item.get('href', '')}")
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            response = await client.get(
                url,
                headers={
                    "User-Agent": "FRIDAY Local Agent/0.1 (+https://localhost)"
                },
            )
        ...
        return ToolResult(
            success=True,
            output=compact_text,
            metadata={"title": title, "url": url, "length": len(compact_text)},
        )
